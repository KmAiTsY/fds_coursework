		...
		<div class="row p-4">
			<div class="col">
				<ul class="list-group list-group-horizontal">
					<li class="list-group-item active"><a href="#" class=" text-white">Замовлення</a></li>
					<li class="list-group-item "><a href="pnl_catalog">Каталог</a></li>
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							if ($ordr_stat[0] == 'Ready') {
								if ($inv_type[0] == 'paypal') {
									...
									echo '<td><a href="pnl_order?act=paid&id=' . encryptIt($row['ordr_id']) . '" onclick="return confirm()">Позначити оплаченим</a><br>
									<a href="pnl_order?act=delete&id=' . encryptIt($row['ordr_id']) . '" onclick="return confirm()">Видалити замовлення</a></td>';
									echo '</tr>';
								}
							} else if ($ordr_stat[0] == 'Completed') {
								echo '<td><a href="pnl_order?act=delete&id=' . encryptIt($row['ordr_id']) . '" onclick="return confirm()">Видалити замовлення</a></td>';
								echo '</tr>';
							} else {
								echo '<td><a href="pnl_order?act=ready&id=' . encryptIt($row['ordr_id']) . '" onclick="return confirm()">Позначити як готовий</a><br>
								<a href="pnl_order?act=delete&id=' . encryptIt($row['ordr_id']) . '" onclick="return confirm()">Видалити замовлення</a></td>';
								echo '</tr>';
							}
						}

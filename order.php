...
<body class="content bg-white">
	...
	<div class="container my-5">
		...
		<h2 class="display-4"><i class="fas fa-tasks"></i> Статус замовлень</h2>

		<div class="row px-3">

			<div class="col">
				<div class="row border-top py-3 font-weight-bold">
					<div class="col-6">Інформація товару</div>
				...
				if (isset($usr_id)) {
					...
					if (mysqli_num_rows($result) > 0) {
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							echo '<div class="col-2">' . number_format((float)($row_data['ctlog_prc']), 2, '.', '') . ' грн</div>';
							echo '<div class="col-2">(' . $row['ordr_qty'] . ')x замовлень</div>';
							if ($row['ordr_stat'] != "") {
								echo '<div class="col-2">' . $row['ordr_stat'] . '</div>';
							} else {
								echo '<div class="col-2">Підготовка</div>';
							}
							echo '</div>';
						...
						if ($payment_type == 'paypal') {
							echo '<h4 class="col text-center text-primary">RM ' . number_format((float)(round($tot_prc + $tot_svc, 1)), 2, '.', '') . ' (Оплачено)</h4>';
							echo '</div>';

							echo '<div class="row">';
							echo '<p class="col text-center ">Оплата через PayPal. </p>';
						} else {
							echo '<h4 class="col text-center text-success">' . number_format((float)(round($tot_prc + $tot_svc, 1)), 2, '.', '') . ' грн(Не оплачено)</h4>';
							echo '</div>';

							echo '<div class="row">';
							echo '<p class="col text-center ">Оплата готівкою. </p>';
						}
						echo '</div>';

						echo '<div class="row pb-5">';
						echo '<small class="col text-center text-muted">Note: Будь ласка, підготуйте невеликі зміни та дотримуйтесь точної загальної ціни.</div>';
						echo '</small>';
					} else {
						echo '<div class="row border-top py-3">';
						echo '<div class="col"> Немає товарів в замовленнях.</div>';
						echo '</div>';
					}
				} else {
					echo '<div class="row border-top py-3">';

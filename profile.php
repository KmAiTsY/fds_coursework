...
<body class="content">
        ...
        <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
            ...
            <div class="my-2 my-lg-0">
                <ul class="navbar-nav ml-auto">
                    ...
                    ?>
                        <a href="index?act=login" class="btn btn-outline-success my-2 my-sm-0">Авторизуватись</a>
                    <?php
                    }
                    ?>
                </ul>
    ...
    <div class="container my-5">
        ...
        <h2 class="display-4 text-white">Профіль</h2>

        <div class="row bg-light rounded p-5">

            <div class="col-4 " id="profile_display">
                <h4 class="pb-3 text-capitalize">Привіт, <?php echo $_SESSION["sess_username"]; ?>.</h4>
                <div class="form-group ">
                    <div class="mb-1 "><i class="fas fa-user"></i> Ім'я</div>
                    <div class="text-muted  text-capitalize"> <?php echo $_SESSION["sess_fullname"]; ?></div>
                </div>
                <div class="form-group ">
                    <div class="mb-1 "><i class="fas fa-map-marked-alt"></i> Адреса</div>
                    <div class="text-muted  text-capitalize"> <?php echo $_SESSION["sess_address"]; ?></div>
                </div>
                <div class="form-group ">
                    <a href="profile?act=update"><small>Змінити профіль</small></a>
                </div>
            </div>

            <div class="col-4 " id="update_display" style="display: none;">
                <form action="" method="POST" id="update_form">
                    <h4 class="pb-3 text-capitalize">Оновити свій профіль.</h4>
                    <div class="form-group font-weight-light">
                        <div class="mb-1 "><i class="fas fa-user"></i> Ім'я</div>
                        <div class="text-muted  text-capitalize"> </div>
                        <div class="input-group">
                            <input type="text" class="form-control" name="fullname" placeholder="Задайте ім'я" value=" <?php echo $_SESSION["sess_fullname"]; ?>" required>
                        </div>
                    </div>
                    <div class="form-group font-weight-light">
                        <div class="mb-1 "><i class="fas fa-map-marked-alt"></i> Адреса</div>
                        <textarea class="form-control" name="address" placeholder="Максимум 50 символів" row="2" cols="50" maxlength="50" required><?php echo $_SESSION["sess_address"]; ?></textarea>
                    </div>
                    <div class="form-group font-weight-light">
                        <div class="mb-1 "><i class="fas fa-user-alt"></i> Юзернейм</div>
                        <input type="text" class="form-control" name="usr" id="usr" placeholder="задайте бзернейм" value="<?php echo $_SESSION["sess_username"]; ?>" required>
                    </div>
                    <div class="form-group font-weight-light">
                        <div class="mb-1 "><i class="fas fa-lock"></i> Пароль</div>
                        <input type="password" class="form-control" name="pwd" id="pwd" placeholder="Введіть свій пароль" value="" required>
                    </div>
                    <div class="form-group font-weight-light">
                        <input type="hidden" name="update">
                        <button type="submit" class="form-control btn btn-primary">Оновити профіль <i class="fas fa-chevron-right"></i></button>
                    </div>
                </form>
            </div>

            <div class="col border-left">
                <h4 class="pb-2">Поточні замовлення</h4>
                <div class="p-2">
                    <?php
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
							...
							<br><small class="text-muted">' . $row['ordr_qty'] . ' Замовлень</small></div>';
                            echo '</div>';
                        }
                    } else {
                        echo "<p class=''> Немає поточних замовлень.</p>";
                    }
                    ?>
                </div>
                <h4 class="pb-2">Історія замовлень</h4>
                <div class="p-2">
                    <?php
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        echo "<p class=''> Немає замовлень в історії</p>";
                    }
                    ?>
                </div>
            </div>

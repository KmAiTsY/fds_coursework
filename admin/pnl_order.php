			...
			<div class="col">
				...
				<table class="table">
					<tr>
						<th>no.</th>
						<th>Ім'я і адреса</th>
						<th>Товар, маразин і кількість</th>
						<th>Статус</th>
						<th>Дія</th>
					</tr>
					...
					if (mysqli_num_rows($result) > 0) {
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							if ($row['ordr_stat'] == null) {
								echo '<td><p class="text-warning">В процесі</p></td>';
							} else {
								echo '<td><p class="text-success">Готово</p></td>';
							}

							echo '<td><a href="pnl_order?act=ordrrdy&id=' . encryptIt($row['usr_id']) . '" onclick="return confirm()">Позначити готовим</a><br><a href="pnl_order?act=ordrdel&id=' . encryptIt($row['usr_id']) . '" onclick="return confirm()">Видалити замовлення</a></td>';
							echo '</tr>';
						}
					}
